...
#include "Adafruit_INA260.h"                                // Rajout INA260 Coupure Auto Volant
Adafruit_INA260 ina260 = Adafruit_INA260();                 // Déclaration Variable ina260
...
/////////////////////////////////////////////
  ...
  { 
    ...
    if (ina260.begin()) {                                 // Rajout INA260 Coupure Auto Volant : Recherche de l'INA au branchement port Com
    Serial.println("INA260 Present");
   uint8_t CurrentSensor = 1;
  }
  else {uint8_t CurrentSensor = 0;}
  ...
  void loop()
  {
    ...
    if (currentTime - lastTime >= LOOP_TIME)
    {
      ...
      // Current (POWER) sensor?                                     // Rajout INA260 Coupure Auto Volant : Ici utiliation de l'etiquette AOG Courant comme Puissance. A devient W
      if (steerConfig.CurrentSensor)
      {
        ...
        if ((abs(analogValue)/ 1000) >= (consigne*( 1.2 + (abs(pwmDisplay) / (steerSettings.highPWM - steerSettings.lowPWM))* 1.1)) )                   // Rajout INA260 Coupure Auto Volant : Si valeur mesurée en A supérieure à la consigne en A alors désengament et Potatofarmer uturn stop function
        {
          steerSwitch = 1; // reset values like it turned off
          currentState = 1;
          previous = 0;
